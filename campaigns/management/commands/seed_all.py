...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Current state: {current_accounts} accounts, {current_campaigns} campaigns"
        )
        ...
        if need_accounts:
            ...
            self.stdout.write(
                f"Accounts already exist ({current_accounts} found), skipping account seeding"
            )
        ...
        if need_campaigns:
            ...
            if Account.objects.exists():
                ...
                self.stdout.write(
                    self.style.ERROR("No accounts found, cannot create campaigns")
                )
        else:
            self.stdout.write(
                f"Campaigns already exist ({current_campaigns} found), skipping campaign seeding"
            )
